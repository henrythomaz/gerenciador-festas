 ...
 * @example
 * // Ordenação simples
 * const order1 = ordenacao("nome:ASC");
 * // Resultado: [["nome", "ASC"]]
 * 
 * // Ordenação múltipla
 * const order2 = ordenacao("nome:ASC,email:DESC");
 * // Resultado: [["nome", "ASC"], ["email", "DESC"]]
 * 
 * // Usando em uma consulta
 * const usuarios = await User.findAll({
 *   order: ordenacao("nome:ASC,email:DESC")
 * });
 * 
 * @example
 * // Sem ordenação
 * const order3 = ordenacao("");
 * // Resultado: []
 * 
 * @example
 * // Com validação
 * try {
 *   const order = ordenacao("nome:ASC,email:INVALIDO");
 * } catch (error) {
 *   // Erro: "Direção inválida para 'email': INVALIDO"
 * }
 */
export default function ordenacao(sort?: string): Order {
  if (!sort) return [];
